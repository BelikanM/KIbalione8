...
# Ajout pour accélération des téléchargements
ARIA2_AVAILABLE = shutil.which("aria2c") is not None
if not ARIA2_AVAILABLE:
    st.warning("aria2 non installé. Installez-le avec 'sudo apt install aria2' pour accélérer les téléchargements avec multi-connexions.")
...
try:
    ...
    st.info("hf_transfer activé pour téléchargements accélérés (fallback si aria2 non disponible).")
except ImportError:
    st.warning("hf_transfer non installé. Installez-le avec 'pip install hf_transfer' pour accélérer les téléchargements de gros fichiers.")

if not ARIA2_AVAILABLE and not HF_TRANSFER_AVAILABLE:
    st.warning("Ni aria2 ni hf_transfer installés. Les téléchargements seront plus lents pour les gros modèles.")
...
def download_with_aria2(repo_id, cache_dir):
    api = HfApi(token=HF_TOKEN)
    files = api.list_repo_files(repo_id)
    for file in files:
        url = f"https://huggingface.co/{repo_id}/resolve/main/{file}?download=true"
        dir_path = cache_dir / Path(file).parent
        dir_path.mkdir(parents=True, exist_ok=True)
        file_name = Path(file).name
        subprocess.run(["aria2c", "-x16", "-s16", "-k1M", "-d", str(dir_path), "-o", file_name, url], check=True)
    return cache_dir
...
def download_worker(repo_id, result_queue):
    try:
        ...
        model_name = repo_id.replace("/", "--")
        snapshot_path = CACHE_HF / f"models--{model_name}" / "snapshots" / commit_hash
        snapshot_path.mkdir(parents=True, exist_ok=True)
        if ARIA2_AVAILABLE:
            download_with_aria2(repo_id, snapshot_path)
        else:
        ...
        result_queue.put(str(snapshot_path))
    except Exception as e:
        result_queue.put(e)
