...
def deep_binary_investigation(file_bytes: bytes, file_name: str = "unknown") -> str:
    ...
    rag_findings = ""
    if st.session_state.vectorstore:
        for query in rag_queries[:3]:  # Limiter à 3 requêtes pour performance
            try:
                result = search_vectorstore(query)
                if result and len(result) > 50:  # Éviter résultats vides
                    rag_findings += f"🔍 Requête: '{query}'\n"
                    rag_findings += f"   Résultat: {result[:300]}...\n\n"
            except Exception as e:
                rag_findings += f"❌ Erreur requête '{query}': {e}\n"
    else:
        rag_findings = "⚠️ Base vectorielle RAG non disponible\n"
    ...
    if numbers and len(numbers) > 10:
        ...
        ert_rag_findings = ""
        if st.session_state.vectorstore:
            for query in ert_queries:
                try:
                    result = search_vectorstore(query)
                    if result and len(result) > 50:
                        ert_rag_findings += f"🔍 {query}:\n   {result[:200]}...\n\n"
                except:
                    pass
        
        if ert_rag_findings:
            investigation_report += "📚 CONNAISSANCES ERT DE LA BASE:\n"
...
Détection ERT:
{ert_detection[:500] if numbers and len(numbers) > 10 else 'N/A'}
